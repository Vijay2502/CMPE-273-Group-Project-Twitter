...
class UserBody extends Component {
    ...
    displayUserProfile(id) {
        console.log("ID >>>>>> ", id);
            try {
                document.querySelector("#root > div > div > div > div > div.col-lg-3 > div > div > div > button:nth-child(11)").setAttribute("data-user-profile-props", JSON.stringify(this.props.user));
        ...
        <div class="list-group">
            <UserBox>
                <button type="button" className="inner-body list-group-item list-group-item-action" onClick={(e) => this.displayUserProfile(this.props)}>
                     {this.props.user.data && this.props.user.data.image ? (<Image image={this.props.user.data.image} />) : (<Image image="https://thefader-res.cloudinary.com/private_images/w_760,c_limit,f_auto,q_auto:best/TwitterLogo__55acee_jntmic/twitter-applications-verified.jpg" />)}
                    <div className="body">
